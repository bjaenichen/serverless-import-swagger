...
module.exports.exec = function () {
  ...
  // import options
  .option('-H, --handler <path>', 'Override default handler.{functionName} with fixed value.').option('-s, --serverless', 'Generate serverless file only.').parse(process.argv);
    ...
    return loadCommonConfig(options).then(function (common) {
      return Promise.all(convertedConfigs.map(function (converted) {
        return Promise.resolve().then(function () {
          return !options.configOnly ? generateServiceIfNeed(converted, options) : converted;
